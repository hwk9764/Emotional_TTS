...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        sz_b, len_q, _ = q.size()   # _은 embedding size일 듯
        sz_b, len_k, _ = k.size()
        sz_b, len_v, _ = v.size()

        residual = q

        # w_qs를 거친 후 output은 d_emb, d_head, input_length
        # b_size, input_length, embedding size(d_model)를 이렇게 쪼갬 => 병렬연산 하기 위해
